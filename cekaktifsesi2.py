...
WITH_2FA_FOLDER = 'a38_sessions/with_2fa'
WITHOUT_2FA_FOLDER = 'a38_sessions/without_2fa'
...
def check_session_status(session_name, active_numbers):
    ...
    try:
        ...
        try:
            ...
            logging.warning(f"⚠️ Gagal a38_sessions 2FA {session_name}: diasumsikan TIDAK 2FA - {e}")
            return "active", "without_2fa"
        ...
        logging.error(f"❌ Error saat a38_sessions session {session_name}: {e}")
        return "corrupt", None
    finally:
...
def check_all_sessions_status():
    ...
    logging.info(f"🚀 Memulai pengea38_sessionsan {len(sessions)} sesi...")
