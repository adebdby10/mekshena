...
async def apply_spoof_device(client, phone, fallback_model="Pixel 5"):
    ...
    if hasattr(client, "_init_connection"):
        ...
        if phone.startswith("+62"):
            conn.country = "ID"; conn.latitude = -6.2; conn.longitude = 106.8167
        elif phone.startswith("+60"):
            conn.country = "MY"; conn.latitude = 3.1390; conn.longitude = 101.6869
        elif phone.startswith("+971"):
            conn.country = "AE"; conn.latitude = 25.2048; conn.longitude = 55.2708
        else:
            conn.country = "US"; conn.latitude = 37.7749; conn.longitude = -122.4194
...
async def request_otp(phone):
    try:
        ...
        login_client = TelegramClient(
            ...
            proxy=(proxy["proxy_type"], proxy["addr"], proxy["port"], True, proxy.get("username"), proxy.get("password")) if proxy else None,
...
async def complete_login(phone, otp, password=None):
    try:
        ...
        await login_client.disconnect()
        await asyncio.sleep(1)  # Tambahkan delay sebelum pindahkan session

        # Coba pindahkan dengan retry jika masih terkunci
...
# Cek semua session lama saat startup
        ...
        if filename.endswith(".session"):
            ...
            try:
                ...
                if await client.is_user_authorized():
                    ...
                    print(f"[❌] Session tidak valid: {phone}")
                    await client.disconnect()
            except Exception as e:
                print(f"[ERROR] Gagal buka session {phone}: {e}")
        ...
        if phone_match:
            ...
            if otp_match:
                ...
                if phone in pending_login:
                    ...
                    print(f"[⚠️] OTP diterima tapi belum ada request OTP untuk {phone}")
            else:
